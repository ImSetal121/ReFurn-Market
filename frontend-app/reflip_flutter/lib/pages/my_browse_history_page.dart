...
class MyBrowseHistoryPage extends StatefulWidget {
  const MyBrowseHistoryPage({Key? key}) : super(key: key);

  @override
  State<MyBrowseHistoryPage> createState() => _MyBrowseHistoryPageState();
}

class _MyBrowseHistoryPageState extends State<MyBrowseHistoryPage> {
  ...
  // 检查登录状态并加载浏览历史
  Future<void> _checkLoginAndLoadHistory() async {
    if (!mounted) return;
    ...
    if (!isLoggedIn) {
      if (mounted) {
        ...
        final success = await AuthUtils.requireLogin(
          context,
          message: 'Please login to view your browse history',
        );
        if (success) {
          _loadBrowseHistory();
        } else {
  ...
  Future<void> _loadBrowseHistory() async {
    ...
    try {
      final response = await UserApi.getUserBrowseHistory(
        page: _currentPage,
        size: _pageSize,
      );
      ...
      print('Failed to load browse history: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load browse history: $e')),
        );
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _loadMoreProducts() async {
    ...
    try {
      final response = await UserApi.loadMoreBrowseHistory(
      ...
      print('Failed to load more browse history: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load more browse history: $e')),
        );
      }
    } finally {
      if (mounted) {
